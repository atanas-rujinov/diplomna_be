...
class ArrivalLogger:
    ...
    def __init__(self):
        ...
        self.data_lock = Lock()
        self.csv_lock = Lock()  # FIX: separate lock exclusively for CSV file writes
        ...
        # FIX: open file once and keep it open — avoids interleaved writes from
        # repeated open('a') calls across concurrent async iterations
    ...
    def _init_csv_log(self):
        ...
        try:
            # File is already open; only write header when it has no content yet
    ...
    def close(self):
        """Flush and close the CSV file — call on shutdown"""
        with self.csv_lock:
            try:
                self._csv_file.flush()
                self._csv_file.close()
                logger.info("CSV log file closed cleanly.")
            except Exception as e:
    ...
    def _log_arrival_to_csv(self, now: datetime, trip_id: str, route_id: str,
        ...
        FIX: uses csv_lock to serialise all writes through the single persistent
        file handle, then flushes immediately so no row is left in the OS buffer
        when the process crashes or restarts.
        """
        try:
            with self.csv_lock:
                self._csv_writer.writerow([
                ...
                self._csv_file.flush()  # FIX: guarantee row hits disk before next write
        except Exception as e:
        ...
        try:
            while True:
                try:
                    ...
                    # Track trip_ids present in this feed to update last_seen
                    ...
                    for entity in feed.entity:
                        ...
                        # Get trip_id (used as vehicle_id for consistency)
                    ...
                    # Update last_seen for vehicles still in feed but with no new arrivals
                    with self.data_lock:
            ...
            # FIX: guarantee clean file close even if the loop is cancelled or crashes
            self.close()
